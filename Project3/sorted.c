...
//uses binary search to find the given word in the arrat | O(log n) time complexity
static int search(SET *sp, char* searchWord, bool *found){
	...
	*found = false;
	...
	while(lo <= hi){
		mid = (lo + hi)/2; 
		if(strcmp(searchWord, sp->words[mid]) == 0){
			*found = true;
			return mid;
		}
		else if(strcmp(searchWord, sp->words[mid]) < 0)
			hi = mid-1;
		else if(strcmp(searchWord, sp->words[mid]) > 0)
			lo = mid+1;
	}
...
//O(n) time complexity in the worst case because it calls the seacrh function and must move every element down
void addElement(SET *sp, char *elt){
	...
	if(found == false){
		...
		//first move all the words in the list down to make space for where the new word is being inserted
		//O(n) runtime in the worst case
		for(int i = sp->count; i > idx; i--){
			sp->words[i] = sp->words[i-1];
		}
		
		//insert the new word | O(1) runtime
		sp->words[idx] = malloc(sizeof(char) * (strlen(elt)+1));
...
//O(n) time complexity in the worst case because it must move every element in the array up
...
//O(log n) time complexity because it calls the search function
